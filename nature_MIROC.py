# -*- coding: utf-8 -*-
"""
Created on Mon Aug 14 16:28:33 2017
...
def mainM():
    wbgt, wbgt95 = wbgt_calcM()
    tmeans = monthmeanTallM()
    tx = tmaxM()
    r95 = R95pM()
    r = dailymeanM('pr')
    tbar = dailymeanM('tas')
    total_r, total_t, globprcnts = totalsM()
    prcnts, all_mean = percentsM(tbar, tx, wbgt95, r, r95)
    tx90p = {}
    ...
    a_t, b_t, c_t = mlrclusterM(tmeans, tx90p)
    a_w, b_w, c_w = mlrclusterM(tmeans, wbgt95)
    a_r, b_r, c_r = mlrclusterM(tmeans, r95, msk='TROP')
...
def dailymeanM(item, region=[0, 128, 0, 256]):
    import glob
    ...
    for x, exp in enumerate(exps):
        a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + exp + '/day/' +
                      item + '/*')
...
def dailymean_meanM(item, region=[0, 128, 0, 256]):
    import glob
    from netCDF4 import Dataset
    sindices = np.zeros((10*92))
    for i in range(10):
    ...
    for x, exp in enumerate(exps):
        ...
        for e, d in enumerate(a):
            nc_fid = Dataset(d, 'r')
            sdata += np.mean(nc_fid.variables[item][sindices, region[0]:
                                                    region[1],
                                                    region[2]:region[3]],
                             axis=0)
            print('Done ' + str(item) + ' ' + str(exp) + ' ' + str(e+1))
...
def monthmeanM(item, level, region=[0, 128, 0, 256]):
    import glob
    from netCDF4 import Dataset
    ...
    for x, exp in enumerate(exps):
        a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + exp +
                      '/mon/' + item + '/*')
...
def R95pM(region=[0, 128, 0, 256]):
    ...
    a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/All-Hist/day' +
                  '/pr/*')
...
def tmaxM(region=[0, 128, 0, 256]):
    ...
    a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/All-Hist' +
                  '/day/tasmax/*')
...
def wbgt_calcM():
    ...
    a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + 'All-Hist' +
                  '/day/tas/*')
    b = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + 'All-Hist' +
                  '/day/hurs/*')
    wbgt = np.zeros((2, np.ma.size(a), 128, 256))
    wbgt_95p = np.zeros((2, np.ma.size(a), 128, 256))
    sdataT = np.zeros((3*10*np.ma.size(a), 128, 256))
    wdataT = np.zeros((3*10*np.ma.size(a), 128, 256))
    sdataH = np.zeros((3*10*np.ma.size(a), 128, 256))
    wdataH = np.zeros((3*10*np.ma.size(a), 128, 256))
    wbgts = np.zeros((3*10*np.ma.size(a), 128, 256))
    wbgtw = np.zeros((3*10*np.ma.size(a), 128, 256))
    jan = np.zeros((31*10))
    feb = np.zeros((28*10))
    jun = np.zeros((30*10))
    jul = np.zeros((31*10))
    ...
    for e, d in enumerate(a):
        nc_fid = Dataset(d, 'r')
        sdataT_d = nc_fid.variables['tas'][sindices, :]
        wdataT_d = nc_fid.variables['tas'][windices, :]
    ...
    for e, d in enumerate(b):
        nc_fid = Dataset(d, 'r')
        sdataH_d = nc_fid.variables['hurs'][sindices, :]
        wdataH_d = nc_fid.variables['hurs'][windices, :]
    ...
    for x, ep in enumerate(exps):
        a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + ep +
                      '/day/tas/*')
        b = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + ep +
                      '/day/hurs/*')
        wbgt = np.zeros((2, np.ma.size(a), 128, 256))
        wbgt_95p = np.zeros((2, np.ma.size(a), 128, 256))
        sdataT = np.zeros((3*10*np.ma.size(a), 128, 256))
...
def latweightmeanM(data, msk='no'):
    ...
    if msk == 'yes':
        mask = np.zeros((np.ma.size(data, axis=0), 128, 256))
        mask[:] = lsm1
        meaned = np.mean(weighted[mask > 0.5]) / np.mean(meshlatweight[lsm1 > 0.5])
    elif msk == 'NH':
        mask = np.zeros((np.ma.size(data, axis=0), 64, 256))
        lsm1 = lsm1[64:, :]
        mask[:] = lsm1
        ...
        mask = np.zeros((np.ma.size(data, axis=0), 42, 256))
        lsm1 = lsm1[43:85, :]
        mask[:] = lsm1
        weighted = weighted[:, 43:85, :]
        meshlatweight = meshlatweight[43:85, :]
        meaned = np.mean(weighted[mask > 0.5]) / np.mean(meshlatweight[lsm1 > 0.5])
    elif msk == 'MID':
        mask = np.zeros((np.ma.size(data, axis=0), 43, 256))
        lsm1 = lsm1[85:, :]
        mask[:] = lsm1
        weighted = weighted[:,  85:, :]
        meshlatweight = meshlatweight[85:, :]
        meaned = np.mean(weighted[mask > 0.5]) / np.mean(meshlatweight[lsm1 > 0.5])
    elif msk == 'no':
...
def totalsM():
    ...
    for x, exp in enumerate(exps):
        a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + exp +
                      '/pr/*')
        b = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + exp +
                      '/tas/*')
        dataR = np.zeros((365*10*np.ma.size(a), 128, 256))
        dataT = np.zeros((365*10*np.ma.size(b), 128, 256))

        for e, d in enumerate(a):
            nc_fid = Dataset(d, 'r')
            dataR[3650*e:3650*(e+1)] = nc_fid.variables['pr'
                                                        ][:3650, :]
            print('Done R totals ' + str(exp) + ' ' + str(e+1))
        total_r[x, 0, 0] = latweightmeanM(dataR, msk='no') * 86400
        total_r[x, 0, 1] = latweightmeanM(dataR[windices, :], msk='no') * 86400
        total_r[x, 0, 2] = latweightmeanM(dataR[sindices, :], msk='no') * 86400
        total_r[x, 1, 0] = latweightmeanM(dataR, msk='yes') * 86400
        dataRw = dataR[windices, :]
        dataRs = dataR[sindices, :]
        total_r[x, 1, 1] = latweightmeanM(dataRw, msk='yes') * 86400
        total_r[x, 1, 2] = latweightmeanM(dataRs, msk='yes') * 86400
        '''
        dataR_point = np.zeros((np.ma.size(a)*31280))
        for l in range(np.ma.size(a)*31280):
            dataR_point[l] = latweightmean(dataR[l, :]) * 86400
        dataR_11 = np.zeros((np.ma.size(a)))
        for y in range(np.ma.size(a)):
            dataR_11[y] = np.mean(dataR_point[31280*y:31280*(y+1)], axis=0)
        '''
        dataR_11 = np.zeros((2, np.ma.size(a)))
        for y in range(np.ma.size(a)):
            dataR_11[0, y] = latweightmeanM(dataR[3650*y:3650*(y+1)], msk='no') * 86400
            dataR_11[1, y] = latweightmeanM(dataR[3650*y:3650*(y+1)], msk='yes') * 86400
        ...
        for f, g in enumerate(b):
            nc_fid = Dataset(g, 'r')
            dataT[3650*f:3650*(f+1)] = nc_fid.variables['tas'
                                                        ][:3650, :]
            print('Done T totals ' + str(exp) + ' ' + str(f+1))
        total_t[x, 0, 0] = latweightmeanM(dataT, msk='no')
        total_t[x, 0, 1] = latweightmeanM(dataT[windices, :], msk='no')
        total_t[x, 0, 2] = latweightmeanM(dataT[sindices, :], msk='no')
        total_t[x, 1, 0] = latweightmeanM(dataT, msk='yes')
        dataTw = dataT[windices, :]
        dataTs = dataT[sindices, :]
        total_t[x, 1, 1] = latweightmeanM(dataTw, msk='yes')
        total_t[x, 1, 2] = latweightmeanM(dataTs, msk='yes')
        '''
        ...
        for y in range(np.ma.size(b)):
            dataT_11[0, y] = latweightmeanM(dataT[3650*y:3650*(y+1)], msk='no')
            dataT_11[1, y] = latweightmeanM(dataT[3650*y:3650*(y+1)], msk='yes')
...
def monthmeanTallM():
    ...
    for x, exp in enumerate(exps):
        a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + exp +
                      '/day/tas/*')
        data = np.zeros((3650*np.ma.size(a), 128, 256))
        tbar = np.zeros((np.ma.size(a)))
        for e, d in enumerate(a):
            nc_fid = Dataset(d, 'r')
            data[3650*e:3650*(e+1)] = nc_fid.variables['tas'
                                                       ][:3650, :]
            print('Done tas ' + str(exp) + ' ' + str(e+1))
        for y in range(np.ma.size(a)):
            tbar[y] = latweightmeanM(data[3650*y:3650*(y+1), :], msk='no')
        output[exp] = tbar
    return output


def monthmeanTall_summM(mask='MID'):
    import glob
    from netCDF4 import Dataset
    sindices = np.zeros((10*92))
    for i in range(10):
    ...
    for x, exp in enumerate(exps):
        a = glob.glob('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/' + exp +
                      '/day/tas/*')
        data = np.zeros((920*np.ma.size(a), 128, 256))
        tbar = np.zeros((np.ma.size(a)))
        for e, d in enumerate(a):
            nc_fid = Dataset(d, 'r')
            data[920*e:920*(e+1)] = nc_fid.variables['tas'
                                                     ][sindices]
            print('Done tas ' + str(exp) + ' ' + str(e+1))
        for y in range(np.ma.size(a)):
            tbar[y] = latweightmeanM(data[920*y:920*(y+1), :], msk=mask)
        output[exp] = tbar
    return output


def percentsM(tbar, txmax, wbgt_95p, r, r95p):
    exps = ['All-Hist', 'Plus15-Future', 'Plus15-Future_LCO2', 'Plus15-Future_HCO2']
    ...
    for e, exp in enumerate(exps):
        ...
        for i, var in enumerate(varis):
            ...
            for j in range(np.ma.size(var, axis=0)):
                means.append(latweightmeanM(np.expand_dims(var[j, :],
                                                          axis=0), msk='MID'))
                means_trop.append(latweightmeanM(np.expand_dims(var[j, :],
                                                               axis=0),
                                                msk='TROP'))
...
def mlrclusterM(tmeans, varmeans, msk='MID'):
    import pandas as pd
    ...
    def elevenyrmns(data, msk='MID'):
        ...
        for j in range(np.ma.size(data['All-Hist'][1], axis=0)):
            pmean.append(latweightmeanM(np.expand_dims(data['All-Hist']
                                                           [1, j, :],
                                                       axis=0), msk))
        pmean = np.mean(pmean)
        means = []
        ...
        for exp in exps:
            ...
            for j in range(np.ma.size(data[exp][1], axis=0)):
                means.append(latweightmeanM(np.expand_dims(data[exp][1, j, :],
                                                           axis=0), msk)-pmean)
        return means, size
...
def mlrcluster_gridM(tmeans, varmeans):
    import pandas as pd
    from statsmodels.formula.api import ols
    beta = np.zeros((2, 128, 256))
    co2 = np.array([390.4, 423.4, 395.8, 550.0])
    f = 3.71 * np.log(co2/278) / np.log(2)  # 278 as preindust [CO2]
    d_f = f[1:] - f[0]
...
def fig4M(tmeans, conversion, rcpplume, alpha, beta, cvs):
    ...
    tmean[2] = np.mean(tmeans['Plus15-Future_HCO2'])
...
emissions from 2011 (Gt C)', fontsize=20, **hfont)
    ...
    plt.title('MIROC5 WBGT95p', y=1.01, fontsize=20, **hfont)
...
def boxplotprecip_daysM(globprcnts, prcnts):
    from matplotlib import pyplot as plt
    hfont = {'fontname': 'Arial'}
    fig, ax1 = plt.subplots(facecolor='w', edgecolor='k', linewidth=2)
    ...
    ax4 = ax1.twinx()
    for i in range(3):
        ax4.errorbar(x[21+i:30:3], y1[i:9:3]*365, yerr=yerrs1[:, i:9:3]*365,
                     color=color[i], fmt='x', elinewidth=2, capthick=2,
                     capsize=5, ms=10, mew=2)
    ax5 = ax1.twinx()
    for i in range(3):
    ...
    ax1.set_xlim([0, 33.5])
    ax1.set_ylim([.6, 1.8])
    ax2.set_ylim([5.25, 17.75])
    ax3.set_ylim([.9, 2.3])
    ax4.set_ylim([-17.5, 37.5])
    ax5.set_ylim([-0.025, .275])
...
def fig1_daysM(datat, dataw, datar, betat=np.zeros(2), betaw=np.zeros(2),
    ...
    sig0r = ttestM(datar['Plus15-Future_LCO2'][0, :],
                  datar['Plus15-Future_HCO2'][0, :]-betar[0]*dT, siglevel=10)
    sig1r = ttestM(datar['Plus15-Future_LCO2'][1, :],
                  datar['Plus15-Future_HCO2'][1, :]-betar[1]*dT, siglevel=10)
    sig0t = ttestM(datat['Plus15-Future_HCO2'][0, :]-betat[0]*dT,
                  datat['Plus15-Future_LCO2'][0, :], siglevel=10)
    sig1t = ttestM(datat['Plus15-Future_HCO2'][1, :]-betat[1]*dT,
                  datat['Plus15-Future_LCO2'][1, :], siglevel=10)
    sig0w = ttestM(dataw['Plus15-Future_HCO2'][0, :]-betaw[0]*dT,
                  dataw['Plus15-Future_LCO2'][0, :], siglevel=10)
    sig1w = ttestM(dataw['Plus15-Future_HCO2'][1, :]-betaw[1]*dT,
    ...
    ctrst = np.linspace(-10, 10, 17)
    ctrsw = np.linspace(-1.6, 1.6, 17)
...
def ttestM(series1, series2, siglevel=10, testtype='two'):
    """
    ...
    sigarray = np.full((128, 256), siglevel)
    if testtype == 'two':
        a = 2
    elif testtype == 'one':
        a = 1
...
def kstestM(series1, series2, siglevel=10):
    import scipy.stats as st
    sigarray = np.full((128, 256), siglevel)
    p1 = np.zeros((128, 256))
    for i in range(128):
...
def fig1_extraM(datat, datatx, dataw, datar):
    from netcdfread import ncread
    import matplotlib as mpl
    from matplotlib import pyplot as plt
    from mpl_toolkits.basemap import Basemap, shiftgrid, addcyclic
    lat = ncread('/network/aopp/hera/mad/bakerh/HAPPI/MIROC5/All-Hist/tas/tas_Aday_MIROC5_All-Hist_est1_v2-0_run001_20060101-20161231.nc','lat')
    lon = np.arange(0, 360, 360/256)
    #lat = ncread('/network/aopp/hera/mad/bakerh/HAPPI/batch_522/atmos/item15201_monthly_mean/item15201_monthly_mean_a00a_2090-01_2100-12.nc', 'latitude1')
    winterr = np.mean(datar['Plus15-Future_HCO2'][0, :], axis=0) - np.mean(datar['Plus15-Future_LCO2'][0, :], axis=0)
    summerr = np.mean(datar['Plus15-Future_HCO2'][1, :], axis=0) - np.mean(datar['Plus15-Future_LCO2'][1, :], axis=0)
    ...
    wintert = np.mean(datat['Plus15-Future_HCO2'][0, :], axis=0) - np.mean(datat['Plus15-Future_LCO2'][0, :], axis=0)
    summert = np.mean(datat['Plus15-Future_HCO2'][1, :], axis=0) - np.mean(datat['Plus15-Future_LCO2'][1, :], axis=0)
    sig0t = ttestM(datat['Plus15-Future_HCO2'][0, :],
                  datat['Plus15-Future_LCO2'][0, :], siglevel=10)
    sig1t = ttestM(datat['Plus15-Future_HCO2'][1, :],
                  datat['Plus15-Future_LCO2'][1, :], siglevel=10)

    winterw = np.mean(dataw['Plus15-Future_HCO2'][0, :], axis=0) - np.mean(dataw['Plus15-Future_LCO2'][0, :], axis=0)
    summerw = np.mean(dataw['Plus15-Future_HCO2'][1, :], axis=0) - np.mean(dataw['Plus15-Future_LCO2'][1, :], axis=0)
    sig0w = ttestM(dataw['Plus15-Future_HCO2'][0, :],
                  dataw['Plus15-Future_LCO2'][0, :], siglevel=10)
    sig1w = ttestM(dataw['Plus15-Future_HCO2'][1, :],
                  dataw['Plus15-Future_LCO2'][1, :], siglevel=10)

    wintertx = np.mean(datatx['Plus15-Future_HCO2'][0][0, :], axis=0) - np.mean(datatx['Plus15-Future_LCO2'][0][0, :], axis=0)
    summertx = np.mean(datatx['Plus15-Future_HCO2'][0][1, :], axis=0) - np.mean(datatx['Plus15-Future_LCO2'][0][1, :], axis=0)
    sig0tx = ttestM(datatx['Plus15-Future_HCO2'][0][0, :],
                  datatx['Plus15-Future_LCO2'][0][0, :], siglevel=10)
    sig1tx = ttestM(datatx['Plus15-Future_HCO2'][0][1, :],
...
def plotall(data, colorlimit, meaning='mean', mask='yes', precip='no',
    ...
    if meaning == 'mean':
        ...
        sig0 = ttestM(data[b1][0, :],
                     data[b2][0, :], siglevel=10)
        sig1 = ttestM(data[b1][1, :],
        ...
        sig0 = ttestM(data[b2][meaning][0, :],
                     data[b1][meaning][0, :], siglevel=10)
        sig1 = ttestM(data[b2][meaning][1, :],
                     data[b1][meaning][1, :], siglevel=10)
    fig = plt.figure(facecolor='w', edgecolor='k', linewidth=2)
    if mask == 'yes':
        lsm = ncread('/home/bakerh/Documents/DPhil/Python/\
lsm_t85.nc', 'lsm')
        winter = np.ma.masked_array(winter, mask=lsm)
        summer = np.ma.masked_array(summer, mask=lsm)
...
def fig1_ratioM(datat, dataw, datar, d1='Plus20-Future', d2='Plus15-Future',
    ...
    winterr = latcross(datar[d1])[0] - np.mean(latcross(datar[d2])[0], axis=0)
    ...
    winterr_ens = np.mean(latcross(datar[d1])[0], axis=0)-np.mean(latcross(datar[d2])[0], axis=0)
    ...
    summerr = latcross(datar[d1])[1] - np.mean(latcross(datar[d2])[1], axis=0)
    plt.plot(lat, np.transpose(summerr), color='grey', alpha=0.1)
    summerr1 = latcross(datar['Plus15-Future_HCO2']- dT * betar[1])[1]  - np.mean(latcross(datar['Plus15-Future_LCO2'])[1], axis=0)
    plt.plot(lat, np.transpose(summerr1), color='pink', alpha=0.1)

    summerr_ens = np.mean(latcross(datar[d1])[1], axis=0)-np.mean(latcross(datar[d2])[1], axis=0)
    plt.plot(lat, summerr_ens, color='black', label='$2.0^{\circ}$C - $1.5^{\circ}$C')
    summerr_ens1 = np.mean(latcross(datar['Plus15-Future_HCO2']- dT * betar[1])[1], axis=0)-np.mean(latcross(datar['Plus15-Future_LCO2'])[1], axis=0)
