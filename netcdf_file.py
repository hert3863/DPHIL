# -*- coding: utf-8 -*-
"""
Created on Tue May 16 15:43:21 2017
...
class netcdf_file(object):
    ...
    def _write_values(self, values):
        if hasattr(values, 'dtype') and values.dtype.char != '?':
    ...
    def _read_values(self):
        ...
        if typecode is 'b':	# override byte to read as int, not bool (netCDF does not support bool!)
            values = fromstring(values, dtype='>%s%d' % ('i', size))
