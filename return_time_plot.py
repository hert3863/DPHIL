...
def calc_return_times(em, direction="ascending", period=1):
    ...
    ey_data.sort()
    # remove nans
    # ey_data = ey_data[:numpy.count_nonzero(~numpy.isnan(ey_data))]
...
def calc_return_time_confidences(em, direction="ascending",
                                 c=[0.05, 0.95], bsn=1e5):
...
def calc_return_time_confidences_2d(ey_data, direction="ascending",
                                    c=[0.05, 0.95], bsn=1e5):
    # c = confidence intervals (percentiles) to calculate
    # bsn = boot strap number, number of times to resample the distribution
...
def return_time_plot(plt_obj, ens, direction="ascending", period=1, color='k'):
    # first calculate the return time data
    y_data, x_data = calc_return_times(ens, direction, period)
    l1 = plt_obj.semilogx(x_data, y_data, 'ko', marker='o', mec=color, mfc='w')
    return l1

###############################################################################


def multi_return_time_plot(plt_obj, ens, direction="ascending",
                           highlight=-2e20, period=1):
    # first calculate the return time data
    y_data = []
    x_data = []
    for em in ens:
    ...
    for e in range(0, len(y_data)):
        l = plt_obj.semilogx(x_data[e], y_data[e], 'ko', marker='o',
                             mec=colors[e], mfc='w', ms=4.0, zorder=1)
        # add an extra marker at the highlight
        if highlight != -2e20:
            # search for the nearest location
            if direction == "descending":
                fi = numpy.interp([highlight], y_data[e][::-1],
                                  x_data[e][::-1])
            else:
...
def multi_return_time_plot_confidence(ens, direction="ascending",
                                      c=0.05, period=1):
    ...
    for e in range(0, len(y_data)):
        ...
        sp.fill_between(x_data[e], c_data[e][0], c_data[e][1],
                        facecolor=colors[e], edgecolor=colors[e], alpha=0.5)
        lines.append(l)
...
def find_event(ens, ev_period, direction="ascending", period=1):
    # find the event value in the first ensemble and the equivalent return
    # period in the other ensembles
    # first calculate the return time data
    y_data = []
    x_data = []
    for em in ens:
